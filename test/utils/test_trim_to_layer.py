...
def test_trim_to_layer_basic():
    ...
    assert paddle.equal_all(x1, paddle.arange(4))
    assert edge_index1.tolist() == [[1, 2, 3], [0, 1, 2]]
    assert paddle.equal_all(edge_weight1, paddle.arange(3))
    ...
    if paddle_geometric.typing.WITH_PADDLE_SPARSE:
        ...
        assert paddle.equal_all(x1, paddle.arange(4)).item()
        assert row.tolist() == [1, 2, 3]
        assert col.tolist() == [0, 1, 2]
        assert paddle.equal_all(value, paddle.arange(3)).item()
    ...
    assert paddle.equal_all(x2, paddle.arange(3)).item()
    assert edge_index2.tolist() == [[1, 2], [0, 1]]
    assert paddle.equal_all(edge_weight2, paddle.arange(2)).item()
    ...
    if paddle_geometric.typing.WITH_PADDLE_SPARSE:
        ...
        assert paddle.equal_all(value, paddle.arange(2)).item()
    ...
    assert paddle.equal_all(x3, paddle.arange(2)).item()
    assert edge_index3.tolist() == [[1], [0]]
    assert paddle.equal_all(edge_weight3, paddle.arange(1)).item()
    ...
    if paddle_geometric.typing.WITH_PADDLE_SPARSE:
        ...
        assert row.tolist() == [1]
        assert col.tolist() == [0]
        assert paddle.equal_all(value, paddle.arange(1)).item()
...
def test_trim_to_layer_hetero():
    ...
    assert paddle.equal_all(x['v'], paddle.arange(3)).item()
    assert edge_index['v', 'to', 'v'].tolist() == [[1, 2], [0, 1]]
    assert paddle.equal_all(edge_weight['v', 'to', 'v'],
                            paddle.arange(2)).item()
...
def test_trim_to_layer_with_neighbor_loader():
    x = paddle.randn(14, 16)
    edge_index = paddle.to_tensor([
        [2, 3, 4, 5, 7, 7, 10, 11, 12, 13],
        [0, 1, 2, 3, 2, 3, 7, 7, 7, 7],
    ])
    edge_weight = paddle.rand(edge_index.size(1))
