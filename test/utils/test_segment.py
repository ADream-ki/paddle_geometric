...
from paddle_geometric.utils import segment


@withCUDA
@pytest.mark.parametrize('reduce', ['sum', 'mean', 'min', 'max'])
def test_segment(device, reduce):
    src = paddle.randn(shape=[20, 16], device=device)
    ...
    if not paddle_geometric.typing.WITH_PADDLE_SCATTER:
        ...
        expected = getattr(paddle, reduce)(src.view(4, 5, -1), axis=1)
        expected = expected[0] if isinstance(expected, tuple) else expected
        assert paddle.allclose(out[:1], paddle.zeros(1, 16, device=device))
